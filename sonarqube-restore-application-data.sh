...
BACKUP_PATH="/srv/sonarqube-application-data/backups/"
RESTORE_PATH="/bitnami/sonarqube/"
BACKUP_PREFIX="sonarqube-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$SONARQUBE_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$SONARQUBE_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
