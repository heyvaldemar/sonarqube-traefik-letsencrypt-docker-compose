#!/usr/bin/env bash
# sonarqube-restore-application-data.sh [backup-file-name]
#
# Replaces SonarQube's application data with one of the archives the backups
# service wrote.
#
#   ./sonarqube-restore-application-data.sh               list and ask
#   ./sonarqube-restore-application-data.sh <file-name>   restore that one
#
# EVERY PATH, NAME AND CREDENTIAL COMES FROM THE RUNNING BACKUPS CONTAINER.
# The previous version cleared /bitnami/sonarqube/, a Bitnami path this stack
# does not have, then unpacked the archive over the live /opt/sonarqube/data.
# Files created after the backup survived the restore: it merged, it did not
# restore.
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using SonarQube." >&2
  exit 1
fi
