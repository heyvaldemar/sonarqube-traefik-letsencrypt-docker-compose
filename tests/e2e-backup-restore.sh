#!/bin/bash
# End-to-end tests for the sonarqube-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-sonarqube}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-sonarqube-traefik-letsencrypt-docker-compose.yml}"
...
DB_NAME="${SONARQUBE_DB_NAME}"
DB_USER="${SONARQUBE_DB_USER}"
DB_HOST="postgres"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./sonarqube-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./sonarqube-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./sonarqube-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./sonarqube-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
